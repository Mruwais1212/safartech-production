...
@endsection
@section('content')
    <x-page-header :header=" __('dashboard.deduct') . ' ' . __('dashboard.balance')">
        <li><a href="/admin-panel/balance">{{ __('dashboard.view_balances') }}</a></li>
        <li class="active">
            {{ __('dashboard.deduct') }} {{ __('dashboard.balance') }}
        </li>
    </x-page-header>
    ...
    <x-panel-form :title="isset($balance)
        ...
        : __('dashboard.deduct') . ' ' . __('dashboard.balance')" :url="'/admin-panel/deduct-balance'" :object="null">
