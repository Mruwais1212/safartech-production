...
@endsection
@section('content')
<x-page-header :header="__('dashboard.view_group_permissions')" :url="'/admin-panel/privilege-group/create'">
    ...
    <tbody>
        ...
        <tr parent_id="{{ $privilege_group->id }}">
            ...
            <td align="center" class="center">
                <ul class="icons-list">
                    @if (in_array('permission-groups-delete',$my_permissions))
                    <li class="text-danger-600">
                        <a onclick="return false;" privilege_group_id="{{ $privilege_group->id }}"
                            delete_url="/admin-panel/privilege-group/{{ $privilege_group->id }}" class="sweet_warning"
                            href="#"><i class="icon-trash"></i>
                        </a>
                    </li>
                    @endif
                    @if (in_array('permission-groups-edit',$my_permissions))
                    <li class="text-primary-600">
                        <a href="/admin-panel/privilege-group/{{ $privilege_group->id }}/edit">
